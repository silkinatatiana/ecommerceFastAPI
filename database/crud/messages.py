...
@handle_db_errors
async def create_message(db: AsyncSession, sender_id: int, chat_id: int, message: str):
    message_item = Messages(chat_id=chat_id, message=message, sender_id=sender_id)
    db.add(message_item)
    await db.commit()
    return message_item
