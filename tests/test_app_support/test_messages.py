...
class TestMessage:
    ...
    async def test_send_message(
        ...
        response = await client_support.post(
            f"/support/messages/{chat.id}/send", data={"message": message_text}
        )
    ...
    async def test_send_message_negative(
        self,
        client_support: AsyncClient,
        client_any: AsyncClient,
        db: AsyncSession,
    ...
    @pytest.mark.negative
    async def test_send_message_negative2(
        self,
        client_any_support: AsyncClient,
        client_any: AsyncClient,
        db: AsyncSession,
        test_chat,
        user_and_employee_ids,
        ...
        chat = await test_chat(user_id=user_id, employee_id=employee_id)
        message_text = fake.text()
        response = await client_any_support.post(
            f"/support/messages/{chat.id}/send", data={"message": message_text}
        )
