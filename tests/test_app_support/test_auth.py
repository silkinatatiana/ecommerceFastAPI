...
class TestAuth:
    ...
    async def test_update_profile(self, client_support: AsyncClient, db: AsyncSession, test_profile_update):
        ...
        assert token is not None, "Client is not authenticated — token missing in cookies"

        user_id = get_user_id_by_token(token=token)
        updated_user = await get_user(db=db, user_id=user_id)
        assert all(
            new_data[field] == getattr(updated_user, field)
            for field in new_data.keys()
        ), f"Данные профиля не обновлены. Ожидалось: {new_data}, получено: {{k: getattr(updated_user, k, '<missing>') for k in new_data}}"

    @pytest.mark.negative
    async def test_update_profile_negative(self, client_support: AsyncClient, db: AsyncSession):
        response = await client_support.put("/auth/update", json=fake.text())
        assert response.status_code == 422

    @pytest.mark.negative
    async def test_update_profile_negative2(self, unauthorized_client_support: AsyncClient, db: AsyncSession, test_profile_update):
        response = await unauthorized_client_support.put("/auth/update", json=test_profile_update)
